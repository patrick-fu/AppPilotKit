...
fn d0_6e_scanner_accepts_only_when_both_canaries_are_absent() {
    verify_fixture("d0-6e2-secret-surface-both-canaries-absent.json");
}

#[test]
fn d0_6f_replays_finished_at_the_expected_nonce() {
    verify_fixture("d0-6f-immediate-finished-replay.json");
}

#[test]
fn d0_7_rejects_incomplete_installed_and_surface_evidence() {
    verify_fixture("d0-7-missing-helper-and-smoke-artifact.json");
}

#[test]
fn d0_7_rejects_zero_byte_build_artifact_captures() {
    verify_fixture("d0-7b-zero-byte-build-artifacts.json");
}
